...
using UniqueShellPIDL = std::unique_ptr<ITEMIDLIST, ShellPIDLDeleter>;
...
HRESULT HandleSHOpenFolderAndSelectItems(
    ...
    UniqueShellPIDL combinedShortcutPidl;
    PCIDLIST_ABSOLUTE shortcutPidl = nullptr;

    // Start passes a full shortcut PIDL; Search passes its parent and child
    if (childCount == 0 && !childPidls &&
        IsAppResolverCaller(returnAddress)) {
        shortcutPidl = folderPidl;
    } else if (s_isSearchProcess && childCount == 1 && childPidls &&
               childPidls[0]) {
        combinedShortcutPidl.reset(ILCombine(folderPidl, childPidls[0]));
        shortcutPidl = combinedShortcutPidl.get();
    }
    ...
    UniquePIDL targetPidl = ResolveShellLinkTarget(shortcutPidl);
    if (!targetPidl || ILIsEqual(shortcutPidl, targetPidl.get())) {
        return originalFunction(folderPidl, childCount, childPidls, flags);
    }

    LogRedirect(shortcutPidl, targetPidl.get());
